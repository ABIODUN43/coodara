...
import { GitBranch as RepoIcon, ArrowLeft, Play, AlertTriangle } from "lucide-react";
import { getRepository } from "@/api/repositories";
import { startAnalysis, listAnalyses, getAnalysisResult } from "@/api/analyses";
...
import { getMockAnalysesForRepo, getMockResult } from "@/data/mockAnalyses";

const ACTIVE_STATUSES = new Set(["pending", "queued", "running"]);

function statusLabel(status: Analysis["status"] | "empty") {
  switch (status) {
    case "empty": return "Never analyzed";
...
function runButtonLabel(status: Analysis["status"] | "empty") {
  switch (status) {
    case "empty": return "Run First Analysis";
    case "pending": return "Analysis Pending...";
    case "queued": return "Analysis Queued...";
    case "running": return "Analysis Running...";
    case "completed": return "Run New Analysis";
    case "failed": return "Retry Analysis";
    case "cancelled": return "Run Analysis Again";
  }
}

function maintBand(v: number) {
  if (v >= 80) return { label: "Healthy", color: "var(--cd-good)" };
  if (v >= 60) return { label: "Watch", color: "var(--cd-warn)" };
  return { label: "At risk", color: "var(--cd-risk)" };
}
...
export function AnalysisPage() {
  const { orgId, repoId } = useParams<{ orgId: string; repoId: string }>() as {
    orgId: string;
    repoId: string;
  };
  ...
  async function loadAll() {
    ...
    if (USE_MOCK_ANALYSIS_DATA) {
      const mockRepo = getMockRepository(repoId);
      if (!mockRepo) {
    ...
    setLoadError(null);
    try {
      const [repoData, list] = await Promise.all([
      ...
      setLoadError("Couldn't load this repository's analysis data.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const polledLatest = useAnalysisPolling(
    ...
    (settled) => {
      setHistory((prev) => [settled, ...prev.filter((h) => h.id !== settled.id)]);
      setLatest(settled);
      if (settled.id === selectedId) loadResultFor(settled);
    }
  );

  const effectiveLatest = USE_MOCK_ANALYSIS_DATA ? latest : (polledLatest ?? latest);
  ...
  async function loadResultFor(analysis: Analysis) {
    ...
    try {
      const r = await getAnalysisResult(orgId, repoId, analysis.id);
      setResult(r);
    } catch {
      setResult(null);
    } finally {
  ...
  useEffect(() => {
    if (effectiveLatest && effectiveLatest.id === selectedId) {
      loadResultFor(effectiveLatest);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [effectiveLatest?.id, effectiveLatest?.status]);

  async function handleSelectHistory(id: number) {
    setSelectedId(id);
    const entry = history.find((h) => h.id === id);
    if (entry) await loadResultFor(entry);
  }
  ...
  async function handleRun() {
    ...
    if (USE_MOCK_ANALYSIS_DATA) {
      ...
      const timer = setInterval(() => {
        ...
        if (progress >= 100) {
          ...
          setLatest(finished);
          setHistory((prev) => [finished, ...prev.filter((h) => h.id !== finished.id)]);
          setResult(getMockResult(7001) ?? null);
        } else {
          setLatest((prev) => (prev ? { ...prev, progress: Math.round(progress) } : prev));
        }
      }, 700);
      return;
    }
    ...
    try {
      const created = await startAnalysis(orgId, repoId);
      ...
      if (err?.response?.status === 409) {
        setConflictNotice("This repository already has an active analysis.");
        await loadAll();
      } else {
        setConflictNotice("Couldn't start a new analysis. Try again.");
      }
    }
  }

  if (loading) {
    return <div className="p-6 text-[13px] text-[var(--cd-ink-soft)]">Loading repository...</div>;
  }
  if (loadError || !repo) {
    return <div className="p-6 text-[13px] text-[var(--cd-risk)]">{loadError ?? "Repository not found."}</div>;
  }

  const displayStatus: Analysis["status"] | "empty" = effectiveLatest?.status ?? "empty";
  const isActive = effectiveLatest ? ACTIVE_STATUSES.has(effectiveLatest.status) : false;
  const runDisabled = isActive;
  const viewingLatest = selectedId === effectiveLatest?.id;
  const selectedEntry = history.find((h) => h.id === selectedId) ?? null;

  const statusColorClass: Record<string, string> = {
    empty: "bg-[var(--cd-sunken)] text-[var(--cd-ink-faint)]",
    pending: "bg-[var(--cd-accent-soft)] text-[var(--cd-accent)]",
    queued: "bg-[var(--cd-accent-soft)] text-[var(--cd-accent)]",
    running: "bg-[var(--cd-accent-soft)] text-[var(--cd-accent)]",
    completed: "bg-[var(--cd-good-bg)] text-[var(--cd-good)]",
    failed: "bg-[var(--cd-risk-bg)] text-[var(--cd-risk)]",
    cancelled: "bg-[var(--cd-sunken)] text-[var(--cd-ink-faint)]",
  };
  ...
  return (
    <div className="px-4 pb-10 pt-4 sm:px-6">
      ...
      <div className="mb-4 flex gap-1 border-b border-[var(--cd-border)]">
        <span className="cursor-default border-b-2 border-[var(--cd-accent)] px-1 pb-2.5 pt-2 mr-[22px] text-[13px] font-semibold text-[var(--cd-ink)]">
          Analysis
        </span>
        <Link
          to={`/dashboard/organizations/${orgId}/repositories/${repoId}/architecture`}
          className="border-b-2 border-transparent px-1 pb-2.5 pt-2 mr-[22px] text-[13px] font-medium text-[var(--cd-ink-faint)] hover:text-[var(--cd-ink)]"
        >
          Architecture
        </Link>
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-start justify-between gap-4 rounded-xl border border-[var(--cd-border)] bg-[var(--cd-surface)] p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="font-mono text-[12px] text-[var(--cd-ink-faint)]">{repo.full_name}</div>
            <div className="mt-2">
              <span
              ...
              >
                <span
                  className={`h-1.5 w-1.5 rounded-full ${displayStatus === "running" ? "animate-pulse" : ""}`}
                  style={{ background: statusDotClass[displayStatus] }}
                />
        ...
        <div className="flex flex-col items-start gap-1.5 sm:items-end">
          ...
          {effectiveLatest?.completed_at && (
            <span className="text-[11.5px] text-[var(--cd-ink-faint)]">
              Last analyzed {formatDate(effectiveLatest.completed_at)}
            </span>
          )}
        </div>
      </div>
      ...
      {displayStatus === "empty" && (
        <div className="flex flex-col items-center rounded-xl border border-[var(--cd-border)] bg-[var(--cd-surface)] px-6 py-16 text-center">
          ...
          <div className="mb-1.5 text-[15px] font-semibold text-[var(--cd-ink)]">Analyze your repository</div>
          <div className="mb-4 max-w-[380px] text-[12.5px] leading-relaxed text-[var(--cd-ink-soft)]">
            Coodara will inspect your repository structure, dependencies, technologies, and code metrics.
          </div>
          <button
            onClick={handleRun}
      ...
      {isActive && (
        <div className="rounded-xl border border-[var(--cd-border)] bg-[var(--cd-surface)] px-6 py-8 text-center">
          <div className="mb-4 text-[14px] font-semibold text-[var(--cd-ink)]">Analyzing {repo.name}</div>
          <div className="mx-auto mb-2.5 h-2.5 max-w-[420px] overflow-hidden rounded-full bg-[var(--cd-sunken)]">
            <div
              ...
              style={{ width: `${effectiveLatest?.progress ?? 0}%` }}
            />
          </div>
          <div className="font-mono text-[12.5px] font-semibold text-[var(--cd-ink-soft)]">
            {effectiveLatest?.progress ?? 0}%
          </div>
          <div className="mt-1.5 text-[12px] text-[var(--cd-ink-faint)]">Analyzing repository...</div>
        </div>
      )}

      {/* FAILED */}
      {!isActive && effectiveLatest?.status === "failed" && viewingLatest && (
        <div className="mb-1 overflow-hidden rounded-xl border border-[var(--cd-risk)]/30 bg-[var(--cd-surface)]">
          ...
          <div className="p-5">
            <div className="mb-1.5 flex items-center gap-2 text-[14px] font-semibold text-[var(--cd-ink)]">
              ...
              We couldn't complete the analysis of this repository.
            </div>
            <div className="mb-3.5 max-w-[520px] text-[12.5px] leading-relaxed text-[var(--cd-ink-soft)]">
              This analysis failed {formatDate(effectiveLatest.updated_at)}.
            </div>
      ...
      {/* Selected historical entry that failed, when not viewing latest */}
      {selectedEntry && selectedEntry.status === "failed" && !viewingLatest && (
        <div className="mb-4 overflow-hidden rounded-xl border border-[var(--cd-risk)]/30 bg-[var(--cd-surface)]">
          ...
          <div className="p-5">
            <div className="mb-3.5 max-w-[520px] text-[12.5px] leading-relaxed text-[var(--cd-ink-soft)]">
              This analysis failed on {formatDate(selectedEntry.updated_at)}.
            </div>
      ...
      {!viewingLatest && (
        <div className="mb-4 flex items-center justify-between gap-3 rounded-lg border border-[var(--cd-border-soft)] bg-[var(--cd-accent-soft)] px-4 py-2.5 text-[12px] font-medium text-[var(--cd-accent)]">
          <span>Viewing analysis from {selectedEntry ? formatDate(selectedEntry.created_at) : ""}</span>
          <button
            onClick={() => effectiveLatest && handleSelectHistory(effectiveLatest.id)}
              ...
              <div className="mb-4 rounded-xl border border-[var(--cd-border)] bg-[var(--cd-surface)]">
                ...
                <div className="grid grid-cols-2 gap-px overflow-hidden rounded-t-lg bg-[var(--cd-border)] sm:grid-cols-5">
                  {[
                    ...
                    ["Lines", result.metrics.loc.toLocaleString()],
                    ["Classes", result.metrics.classes],
                    ["Functions", result.metrics.functions],
                    ["Complexity", result.metrics.complexity],
                  ].map(([label, value]) => (
                    <div key={label as string} className="bg-[var(--cd-surface)] p-4">
                      <span className="text-[11px] font-medium text-[var(--cd-ink-faint)]">{label}</span>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 border-t border-[var(--cd-border-soft)] p-5">
                  <div>
                    <div className="text-[12px] font-semibold text-[var(--cd-ink-soft)]">Maintainability</div>
                    <div className="mt-0.5 text-[11px] text-[var(--cd-ink-faint)]">
                      {maintBand(result.metrics.maintainability).label}
                    </div>
                    <div className="mt-2 h-2 w-[220px] max-w-full overflow-hidden rounded-full bg-[var(--cd-sunken)]">
                      <div
                        ...
                        style={{
                          ...
                          background: maintBand(result.metrics.maintainability).color,
                        }}
                      />
                    </div>
                  </div>
                  <div
                    ...
                    style={{ color: maintBand(result.metrics.maintainability).color }}
                  >
                    {result.metrics.maintainability.toFixed(1)}
                    <span className="text-[13px] font-medium text-[var(--cd-ink-faint)]">/100</span>
                  </div>
                </div>
              </div>
                ...
                <div className="overflow-x-auto p-5">
                  {result.dependency_graph ? (
                    <pre className="whitespace-pre-wrap break-all font-mono text-[11.5px] leading-relaxed text-[var(--cd-ink-soft)]">
                      {(() => {
                        try {
                          return JSON.stringify(JSON.parse(result.dependency_graph.graph_data), null, 2);
                        } catch {
                          return result.dependency_graph!.graph_data;
                        }
                      })()}
                    </pre>
                  ) : (
                    <span className="text-[12.5px] text-[var(--cd-ink-faint)]">
                      No dependency graph data for this analysis.
                    </span>
                  )}
                </div>
              </div>
      ...
      <div className="overflow-hidden rounded-xl border border-[var(--cd-border)] bg-[var(--cd-surface)]">
        ...
        {history.length === 0 ? (
          <div className="p-5 text-[12.5px] text-[var(--cd-ink-faint)]">No analyses yet.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  {["Date", "Status", "Progress"].map((h) => (
                    <th
                      key={h}
              ...
              <tbody>
                {history.map((h) => (
                  <tr
                    ...
                    className={`cursor-pointer ${h.id === selectedId ? "bg-[var(--cd-accent-soft)]" : "hover:bg-[var(--cd-sunken)]"}`}
                  >
                    ...
                    <td className="border-b border-[var(--cd-border-soft)] px-5 py-2.5">
                      <span
                        ...
                        style={{ color: statusDotClass[h.status] }}
                      >
                        <span className="h-1.5 w-1.5 rounded-full" style={{ background: statusDotClass[h.status] }} />
                        {statusLabel(h.status)}
                      </span>
                    </td>
                    <td className="border-b border-[var(--cd-border-soft)] px-5 py-2.5 font-mono text-[12.5px] text-[var(--cd-ink)]">
                      {h.status === "completed" ? "100%" : `${h.progress}%`}
                    </td>
                  </tr>
                ))}
              </tbody>
